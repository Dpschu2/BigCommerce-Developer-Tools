...
Features:
    -Adds 'themes', 'change store', 'web pages', and 'product categories' buttons to BigCommerce sidebar
...
if (scriptAutoUrl.includes('.mybigcommerce.com/manage')) {
    waitForEl('.cp-nav-header-items', () => {
        ...
        `);
        const webPagesUrl = document.querySelector('[href="/manage/content/pages"]') ? '/manage/content/pages' : '/manage/channel/1/pages';
        const catsUrl = '/manage/products/categories';
        const footerContainer = document.querySelector('.cp-nav-footer');
        if (footerContainer) {
            footerContainer.innerHTML = `
                <div class="bc-side-links">
                    <a class="cp-nav-link cp-nav-link-track cp-nav-header-link" href="${webPagesUrl}">Web Pages</a>
                    <a class="cp-nav-link cp-nav-link-track cp-nav-header-link" href="${catsUrl}">Product Categories</a>
                </div>
